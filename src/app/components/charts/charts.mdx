...
import { ThemedChart, Link, Text, Code, Button, ToggleList, Spacer, useTheme, GraphDataView } from 'components';
...
<Playground
  ...
  desc="A multi line chart example with different colors and legends"
  scope={{ ThemedChart, useTheme, Button, useState, ToggleList }}
  code={`() => {
      const min = 1;
      const max = 100;
      const theme = useTheme();
      ...
      return (
          <>
            ...
            <ThemedChart viewMode={dataViewMode}>
...
<Attributes.Title>ThemedChartSeriesProps</Attributes.Title>

```ts
type ThemedChartSeriesProps = {
