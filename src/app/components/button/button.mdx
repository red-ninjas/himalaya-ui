...
<Grid.Container gap={1.5}>
  ...
  <Grid><Button auto p={0}>NoPadding</Button></Grid>
  <Grid><Button auto font={0.25}>Small Font</Button></Grid>
  <Grid><Button auto pl={0} ml="25px">Offset</Button></Grid>
  <Grid xs={12}><Button w="100%" auto={{ xs: false, md: true, lg: false }}>Full Width</Button></Grid>
  <Grid xs={12}>
    <Button auto w="30%" mx="5px">proportion</Button>
    <Button auto w="30%" mx="5px">proportion</Button>
    <Button auto w="30%" mx="5px">proportion</Button>
  </Grid>
</Grid.Container>
...
<Attributes edit="/app/en-us/components/button.mdx">
...
| **ref**       | forwardRef                                | `Ref<HTMLButtonElement or null>` | -                           | -         |
| ...           | native props                              | `ButtonHTMLAttributes`           | `'id', 'className', ...`    | -         |

<Attributes.Title>ButtonTypes</Attributes.Title>

```ts
type ButtonTypes = 'default' | 'primary' | 'tertiary' | 'secondary' | 'success' | 'warning' | 'error' | 'abort';
```
