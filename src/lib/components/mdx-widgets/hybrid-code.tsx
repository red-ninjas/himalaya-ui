...
const extractFileName = (children: ReactNode | undefined, stopDeep: boolean = false): HybridCodeChildrenAndName => {
  ...
  const next = React.Children.map(children, child => {
    ...
    const { children: puredGrandson, name: puredName } = extractFileName(child.props?.children, true);
    if (!puredName) return child;

    name = puredName;
    const withoutSpaceAndNull = React.Children.toArray(puredGrandson).filter((r, index) => {
      if (index === 0 && r === '\n') return false;
      return !!r;
    });
...
const HybridCode: React.FC<HybridCodeProps> = ({ children }) => {
  const { children: withoutNameChildren, name } = useMemo<HybridCodeChildrenAndName>(() => extractFileName(children), [children]);
