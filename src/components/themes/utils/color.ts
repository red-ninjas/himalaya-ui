...
export class Color {
  ...
  constructor(value: string | RGB | HSL) {
    if (typeof value === 'string' && /rgb\(/.test(value)) {
      ...
      value = { r: parseInt(matches ? matches[0] || '0' : '0', 10), g: parseInt(matches[1], 10), b: parseInt(matches[2], 10) };
    } else if (typeof value === 'string' && /hsl\(/.test(value)) {
      const matches = /hsl\((\d{1,3}), ?(\d{1,3}%), ?(\d{1,3}%)\)/.exec(value) ?? [];
      value = { h: parseInt(matches ? matches[0] || '0' : '0', 10), s: parseInt(matches[1], 10), l: parseInt(matches[2], 10) };
    }
