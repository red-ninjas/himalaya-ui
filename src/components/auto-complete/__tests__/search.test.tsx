...
describe('AutoComplete Search', () => {
  ...
  it('should ignore events when disabled', () => {
    let value = '';
    const wrapper = mount(<AutoComplete disabled options={mockOptions} onChange={val => (value = val)} />);
    wrapper.find('input').at(0).simulate('focus');
    wrapper.find('.item').at(0).simulate('click', nativeEvent);
  ...
  it('should trigger select and change handler', () => {
    ...
    const wrapper = mount(<AutoComplete options={mockOptions} initialValue="value" onSelect={selectHandler} onChange={changeHandler} />);
    wrapper.find('input').at(0).simulate('focus');
    wrapper.find('.item').at(0).simulate('click', nativeEvent);
  ...
  it('should work with custom options', () => {
    ...
    const options = mockOptions.map(({ label, value }) => makeOption(label, value) as typeof AutoComplete.Option);
