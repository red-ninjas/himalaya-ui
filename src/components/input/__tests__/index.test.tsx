...
describe('Input', () => {
  ...
  it('should trigger event when input changed', () => {
    let value = '';
    const callback = jest.fn().mockImplementation((e: React.ChangeEvent<HTMLInputElement>) => (value = e.target.value));
  ...
  it('should trigger icon event', () => {
    const click = jest.fn();
    const wrapper = mount(<Input icon={<span id="test-icon">icon</span>} onIconClick={click} iconClickable />);
  ...
  it('should ignore icon event when input disabled', () => {
    const click = jest.fn();
    const wrapper = mount(<Input icon={<span id="test-icon">icon</span>} onIconClick={click} iconClickable disabled />);
    wrapper.find('#test-icon').simulate('click', nativeEvent);
