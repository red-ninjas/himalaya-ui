...
function buildTimeAxisViewsGetter(
  zOrder: SeriesPrimitivePaneViewZOrder,
): ITimeAxisViewsGetter {
  return (source: IDataSource): readonly IAxisView[] =>
    source.timePaneViews?.(zOrder) ?? []
}
...
export class TimeAxisWidget<HorzScaleItem> implements MouseEventHandlers, IDestroyable {
  ...
  public constructor(
    chartWidget: ChartWidget<HorzScaleItem>,
    ...
    this._chart
      ...
      .subscribe(this._recreateStubs.bind(this), this)
  ...
  public mouseDownEvent(event: TouchMouseEvent): void {
    ...
    if (
      model.timeScale().isEmpty() ||
      !this._chart.options().handleScale.axisPressedMouseMove.time
    ) {
      return
    }
  ...
  public setSizes(
    timeAxisSize: Size,
    leftStubWidth: number,
    rightStubWidth: number,
  ): void {
    ...
    if (this._rightStub !== null) {
      this._rightStub.setSize(
        size({ width: rightStubWidth, height: timeAxisSize.height }),
      )
    }
  }
  ...
  public paint(type: InvalidationLevel): void {
    ...
    if (topTarget !== null) {
      ...
      this._drawLabels(
        [...this._chart.model().serieses(), this._chart.model().crosshairSource()],
        topTarget,
      )
  ...
  private _drawAdditionalSources(
    ...
    axisViewsGetter: ITimeAxisViewsGetter,
  ): void {
  ...
  private _drawBackground({
    ...
    clearRect(
      ...
      bitmapSize.width,
      bitmapSize.height,
      this._chart.model().backgroundBottomColor(),
    )
  }
  ...
  private _drawBorder({
    ...
    if (this._chart.options().timeScale.borderVisible) {
      ...
      const borderSize = Math.max(
        1,
        Math.floor(this._getRendererOptions().borderSize * verticalPixelRatio),
      )
  ...
  private _alignTickMarkLabelCoordinate(
    ...
    if (leftTextCoordinate < 0) {
      ...
      coordinate =
        coordinate - Math.abs(this._size.width - (leftTextCoordinate + labelWidth))
    }
  ...
  private _recreateStubs(): void {
    ...
    const borderVisibleGetter = () => {
      return (
        options.leftPriceScale.borderVisible && model.timeScale().options().borderVisible
      )
    }
    ...
    if (options.leftPriceScale.visible && this._leftStub === null) {
      this._leftStub = new PriceAxisStub(
        'left',
        options,
        params,
        borderVisibleGetter,
    ...
    if (options.rightPriceScale.visible && this._rightStub === null) {
      this._rightStub = new PriceAxisStub(
        'right',
        options,
        params,
        borderVisibleGetter,
