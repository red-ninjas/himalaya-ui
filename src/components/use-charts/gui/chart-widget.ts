...
} from './internal-layout-sizes-hints';
...
export class ChartWidget<HorzScaleItem> implements IDestroyable, IChartWidgetBase {
  ...
  private _traverseLayout(ctx: CanvasRenderingContext2D | null): Size {
    ...
    if (this._isRightAxisVisible()) {
      ...
      const rightAxisBitmapWidth = ensureNotNull(
        firstPane.rightPriceAxisWidget(),
      ).getBitmapSize().width;
  ...
  private _adjustSizeImpl(): void {
    ...
    let timeAxisHeight = timeAxisVisible
      ? Math.max(
          ...
          this._options.timeScale.minimumHeight,
        )
      : 0;
  ...
  private _getMouseEventParamsImpl(
    ...
    if (index !== null) {
      const timePoint = this._model.timeScale().indexToTimeScalePoint(index)
        ?.originalTime;
    ...
    if (!('ResizeObserver' in window)) {
      ...
      this._observer = new ResizeObserver((entries: ResizeObserverEntry[]) => {
        const containerEntry = entries.find(
          (entry: ResizeObserverEntry) => entry.target === this._container,
        );
        if (!containerEntry) {
          return;
        }
...
function shouldSubscribeMouseWheel<HorzScaleItem>(
  options: ChartOptionsInternal<HorzScaleItem>,
): boolean {
