  ...
  implements ISubscription<T1, T2, T3>
{
  ...
  public unsubscribe(callback: Callback<T1, T2, T3>): void {
    const index = this._listeners.findIndex(
      (listener: Listener<T1, T2, T3>) => callback === listener.callback,
    )
    if (index > -1) {
  ...
  public fire(param1: T1, param2: T2, param3: T3): void {
    ...
    this._listeners = this._listeners.filter(
      (listener: Listener<T1, T2, T3>) => !listener.singleshot,
    )
    listenersSnapshot.forEach((listener: Listener<T1, T2, T3>) =>
      listener.callback(param1, param2, param3),
    )
  }
