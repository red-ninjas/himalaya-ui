  ...
  implements ISeriesApi<TSeriesType, HorzScaleItem, TData, TOptions, TPartialOptions>, IDestroyable
{
  ...
  public barsInLogicalRange(range: Range<number> | null): BarsInfo<HorzScaleItem> | null {
    ...
    const correctedRange = new TimeScaleVisibleRange(new RangeImpl(range.from as Logical, range.to as Logical)).strictRange() as RangeImpl<TimePointIndex>;
    ...
    const barsBefore =
      dataFirstBarInRange === null || dataFirstBarInRange.index === dataFirstIndex ? range.from - dataFirstIndex : dataFirstBarInRange.index - dataFirstIndex;
