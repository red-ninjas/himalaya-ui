...
export class PriceFormatter implements IPriceFormatter {
  ...
  private _formatAsDecimal(price: number): string {
    ...
    let fracString = '';
    const fracLength =
      this._fractionalLength !== undefined ? this._fractionalLength : NaN;
    if (base > 1) {
      ...
      fracString =
        ...
        numberToStringWithLeadingZero(
          +fracPart.toFixed(this._fractionalLength) * this._minMove,
          fracLength,
        );
    } else {
      ...
      if (fracLength > 0) {
        fracString =
          formatterOptions.decimalSign + numberToStringWithLeadingZero(0, fracLength);
      }
    }
