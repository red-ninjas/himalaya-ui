...
export class PriceScale {
  ...
  public applyOptions(options: DeepPartial<PriceScaleOptions>): void {
    ...
    if (options.scaleMargins !== undefined) {
      ...
      if (top + bottom > 1) {
        throw new Error(
          `Invalid margins - sum of margins must be less than 1, given=${top + bottom}`,
        )
      }
  ...
  public marks(): PriceMark[] {
    ...
    if (
      this._marksCache !== null &&
      (firstValueIsNull || this._marksCache.firstValueIsNull === firstValueIsNull)
    ) {
  ...
  public scaleTo(x: number): void {
    ...
    let scaleCoeff =
      (this._scaleStartPoint + (this._height - 1) * 0.2) / (x + (this._height - 1) * 0.2)
  ...
  private _formatPrice(price: BarPrice, fallbackFormatter?: IPriceFormatter): string {
    return this._formatValue(
      price,
      this._localizationOptions.priceFormatter,
      fallbackFormatter,
    )
  }
  ...
  private _formatPercentage(
    ...
    return this._formatValue(
      percentage,
      this._localizationOptions.percentageFormatter,
      fallbackFormatter,
    )
  }
}
