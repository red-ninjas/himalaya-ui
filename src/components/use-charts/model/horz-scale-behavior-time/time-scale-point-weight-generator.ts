...
export function fillWeightsForPoints(
  ...
  let prevTime =
    startIndex === 0 ? null : cast(sortedTimePoints[startIndex - 1].time).timestamp;
  ...
  for (let index = startIndex; index < sortedTimePoints.length; ++index) {
    ...
    if (prevDate !== null) {
      currentPoint.timeWeight = weightByTime(
        currentDate,
        prevDate,
    ...
    totalTimeDiff +=
      cast(currentPoint.time).timestamp - (prevTime || cast(currentPoint.time).timestamp);
  ...
  if (startIndex === 0 && sortedTimePoints.length > 1) {
    ...
    const approxPrevDate = new Date(
      (cast(sortedTimePoints[0].time).timestamp - averageTimeDiff) * 1000,
    );
