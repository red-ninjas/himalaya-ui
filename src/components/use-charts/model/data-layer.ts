  ...
  if (firstAndLastTime !== undefined && prevFirstAndLastTime !== undefined) {
    return {
      lastBarUpdatedOrNewBarsAddedToTheRight:
        firstAndLastTime.lastTime >= prevFirstAndLastTime.lastTime && firstAndLastTime.firstTime >= prevFirstAndLastTime.firstTime,
    };
  }
  ...
  public setSeriesData<TSeriesType extends SeriesType>(
    ...
    if (isTimeScaleAffected) {
      ...
      newTimeScalePoints.sort(
        (t1: InternalTimeScalePoint, t2: InternalTimeScalePoint) => this._horzScaleBehavior.key(t1.time) - this._horzScaleBehavior.key(t2.time),
      );
