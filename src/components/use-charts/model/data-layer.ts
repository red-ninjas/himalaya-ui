...
export type TimedData<HorzScaleItem> = Pick<
...
function seriesRowsFirsAndLastTime<TSeriesType extends SeriesType, HorzScaleItem>(
...
function timeScalePointTime<TSeriesType extends SeriesType, HorzScaleItem>(
  mergedPointData: Map<
    Series<TSeriesType>,
    SeriesPlotRow<TSeriesType> | WhitespacePlotRow
  >,
): HorzScaleItem {
...
export class DataLayer<HorzScaleItem> {
  ...
  private _seriesRowsBySeries: Map<Series<SeriesType>, SeriesPlotRow<SeriesType>[]> =
    new Map();
  private _seriesLastTimePoint: Map<Series<SeriesType>, InternalHorzScaleItem> =
    new Map();
  ...
  public setSeriesData<TSeriesType extends SeriesType>(
    ...
    if (data.length !== 0) {
      const originalTimes = data.map(
        (d: SeriesDataItemTypeMap<HorzScaleItem>[TSeriesType]) => d.time,
      );

      const timeConverter = this._horzScaleBehavior.createConverterToInternalObj(data);

      const createPlotRow = getSeriesPlotRowCreator<TSeriesType, HorzScaleItem>(
        series.seriesType(),
      );
      ...
      seriesRows = data.map(
        (item: SeriesDataItemTypeMap<HorzScaleItem>[TSeriesType], index: number) => {
    ...
    return this._getUpdateResponse(
      ...
      seriesUpdateInfo(
        this._seriesRowsBySeries.get(series),
        prevSeriesRows,
  ...
  public updateSeriesData<TSeriesType extends SeriesType>(
    ...
    if (
      ...
      throw new Error(
        `Cannot update oldest data, last time=${lastSeriesTime}, new time=${time}`,
      );
    }

    let pointDataAtTime = this._pointDataByTimePoint.get(
    ...
    const createPlotRow = getSeriesPlotRowCreator<SeriesType, HorzScaleItem>(
      series.seriesType(),
    );
    const dataToPlotRow = series.customSeriesPlotValuesBuilder();
    const customWhitespaceChecker = series.customSeriesWhitespaceCheck<HorzScaleItem>();
  ...
  private _updateLastSeriesRow(
    ...
    if (
      lastSeriesRow === null ||
      this._horzScaleBehavior.key(plotRow.time) >
        this._horzScaleBehavior.key(lastSeriesRow.time)
    ) {
  ...
  private _replaceTimeScalePoints(newTimePoints: InternalTimeScalePoint[]): number {
    ...
    for (
      ...
      if (
        this._horzScaleBehavior.key(oldPoint.time) !==
        this._horzScaleBehavior.key(newPoint.time)
      ) {
  ...
  private _getUpdateResponse(
    ...
    if (firstChangedPointIndex !== -1) {
      ...
      this._seriesRowsBySeries.forEach(
        (data: SeriesPlotRow<SeriesType>[], s: Series<SeriesType>) => {
...
function assignIndexToPointData(pointData: TimePointData, index: TimePointIndex): void {
  ...
  pointData.mapping.forEach(
    (seriesRow: Mutable<SeriesPlotRow<SeriesType> | WhitespacePlotRow>) => {
