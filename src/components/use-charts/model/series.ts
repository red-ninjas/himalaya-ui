...
import { TimerId } from '../typings/common';
...
export class Series<T extends SeriesType> extends PriceDataSource implements IDestroyable, ISeries<SeriesType> {
  ...
  public constructor(model: IChartModelBase, options: SeriesOptionsInternal<T>, seriesType: T, _pane?: Pane, customPaneView?: ICustomSeriesPaneView<unknown>) {
    super(model);
  ...
  public override labelPaneViews(_pane?: Pane): readonly IPaneView[] {
    return [this._panePriceAxisView, ...this._customPriceLines.map((line: CustomPriceLine) => line.labelPaneView())];
  }
