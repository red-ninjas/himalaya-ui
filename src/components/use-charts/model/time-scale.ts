...
export class TimeScale<HorzScaleItem> implements ITimeScale {
  ...
  private _formattedByWeight: Map<number, FormattedLabelsCache<HorzScaleItem>> =
    new Map();
  ...
  public timeToIndex(
    ...
    if (
      ...
      this._horzScaleBehavior.key(this._points[this._points.length - 1].time)
    ) {
      // special case
    ...
    const index = lowerbound(
      this._points,
      this._horzScaleBehavior.key(time),
      (a: TimeScalePoint, b: InternalHorzScaleItemKey) =>
        this._horzScaleBehavior.key(a.time) < b,
    );

    if (
      this._horzScaleBehavior.key(time) <
      this._horzScaleBehavior.key(this._points[index].time)
    ) {
  ...
  public isEmpty(): boolean {
    return (
      this._width === 0 || this._points.length === 0 || this._baseIndexOrNull === null
    );
  }
  ...
  public timeRangeForLogicalRange(range: LogicalRange): TimePointsRange {
    ...
    return {
      from: ensureNotNull(
        this.indexToTimeScalePoint(Math.max(firstIndex, from) as TimePointIndex),
      ),
      to: ensureNotNull(
        this.indexToTimeScalePoint(Math.min(lastIndex, to) as TimePointIndex),
      ),
    };
  }
  ...
  public marks(): TimeMark[] | null {
    ...
    const maxLabelWidth =
      pixelsPerCharacter *
      (this._options.tickMarkMaxCharacterLength || defaultTickMarkMaxCharacterLength);
  ...
  public scrollTo(x: Coordinate): void {
    ...
    this._rightOffset =
      ensureNotNull(this._commonTransitionStartState).rightOffset + shiftInLogical;
    this._visibleRangeInvalidated = true;
  ...
  public formatDateTime(timeScalePoint: TimeScalePoint): string {
    if (this._localizationOptions.timeFormatter !== undefined) {
      return this._localizationOptions.timeFormatter(
        timeScalePoint.originalTime as HorzScaleItem,
      );
    }
  ...
  private _setVisibleRange(newVisibleRange: TimeScaleVisibleRange): void {
    ...
    if (
      !areRangesEqual(oldVisibleRange.strictRange(), this._visibleRange.strictRange())
    ) {
      this._visibleBarsChanged.fire();
    }

    if (
      !areRangesEqual(oldVisibleRange.logicalRange(), this._visibleRange.logicalRange())
    ) {
