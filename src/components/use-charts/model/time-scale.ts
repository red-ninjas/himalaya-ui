...
export class TimeScale<HorzScaleItem> implements ITimeScale {
  ...
  public applyOptions(options: DeepPartial<HorzScaleOptions>, _localizationOptions?: DeepPartial<LocalizationOptions<HorzScaleItem>>): void {
  ...
  private _minRightOffset(): number | null {
    ...
    const barsEstimation = this._options.fixLeftEdge ? this._width / this._barSpacing : Math.min(Constants.MinVisibleBarsCount, this._points.length);

    return firstIndex - baseIndex - 1 + barsEstimation;
  }

  private _maxRightOffset(): number {
    return this._options.fixRightEdge ? 0 : this._width / this._barSpacing - Math.min(Constants.MinVisibleBarsCount, this._points.length);
  }
