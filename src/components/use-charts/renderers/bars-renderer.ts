...
export class PaneRendererBars extends BitmapCoordinatesPaneRenderer {
  ...
  protected override _drawImpl({
    ...
    const drawOpenClose =
      ...
      this._data.barSpacing >= Math.floor(1.5 * horizontalPixelRatio);
      ...
      if (drawOpenClose) {
        if (this._data.openVisible) {
          ...
          let openTop = Math.max(
            bodyTop,
            Math.round(bar.openY * verticalPixelRatio) - bodyWidthHalf,
          );
        ...
        let closeTop = Math.max(
          bodyTop,
          Math.round(bar.closeY * verticalPixelRatio) - bodyWidthHalf,
        );
  ...
  private _calcBarWidth(pixelRatio: number): number {
    ...
    return Math.max(
      limit,
      Math.floor(optimalBarWidth(ensureNotNull(this._data).barSpacing, pixelRatio)),
    );
  }
}
