...
export const flattenArray = <T>(contents: T[][] | unknown): T[] => {
  ...
  return contents.reduce((pre, current) => {
    return pre.concat(Array.isArray(current) ? flattenArray(current as any as T[][]) : current);
  }, []);
};
...
export const focusNextElement = (containerElement: HTMLElement | null, done: () => void, isBack?: boolean) => {
