...
export const makeToastActions = (actions: Toast['actions'], cancelHandle: () => void) => {
  const handler = (event: React.MouseEvent<HTMLButtonElement>, userHandler: ToastAction['handler']) => {
...
export const isTopPlacement = (placement: ToastPlacement) => `${placement}`.toLowerCase().startsWith('top');
export const isLeftPlacement = (placement: ToastPlacement) => `${placement}`.toLowerCase().endsWith('left');
