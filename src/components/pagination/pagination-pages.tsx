...
const PaginationPages: React.FC<Props> = ({ limit, count, current, setPage }) => {
  ...
  return (
    <>
      ...
      {showBeforeEllipsis && <PaginationEllipsis key="pagination-ellipsis-before" isBefore onClick={() => setPage(last => (last - 5 >= 1 ? last - 5 : 1))} />}
      {showBeforeEllipsis && showAfterEllipsis ? middlePages : showBeforeEllipsis ? endPages : startPages}
      {showAfterEllipsis && <PaginationEllipsis key="pagination-ellipsis-after" onClick={() => setPage(last => (last + 5 <= count ? last + 5 : count))} />}
