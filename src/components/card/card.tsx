...
const CardComponent: React.FC<React.PropsWithChildren<CardProps>> = ({
  ...
  const { color, bgColor, borderColor } = useMemo(() => getStyles(type, theme.palette, shadow), [type, theme.palette, shadow]);
    ...
    <div className={useClasses('card', className)} {...props}>
      ...
      <style jsx>{`
        .card {
          ...
          border: ${borderSize}px solid ${theme.palette.border.value};
        }
